...
const handleOkClick = async (dossier) => {
  ...
  try {
    const response = await fetch(`https://localhost:8000/retraits/${Id_acteur}/${Id_niveau}/${dossier.Id_affecte}/${dossier.Reference_dossier}`, options);
  ...
  return (
    <div style={pageStyle}>
      ...
      <div style={{ marginLeft: '0px', display: 'flex', textAlign: 'center', fontSize: '25px', fontFamily: 'Century Gothic' }}>Mes affectations</div>
      </div>
      ...
      <div style={tableContainerStyle}>
      <div style={{ marginLeft: '0px', display: 'flex', textAlign: 'left',fontFamily: 'Calibri', fontSize: '18px' }}>Affectations récentes</div>
